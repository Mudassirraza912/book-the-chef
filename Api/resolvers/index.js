...
const { resturantFoods, resturantChefs, userOrders } = require("./merge");
...
module.exports = {
    ...
    allUsers: async (args, req) => {
      try {
        ...
        return users.map((user) => {
          return {
            ...user._doc,
            fetch_orders: userOrders.bind(this, users.orders)
          };
        });
      } catch (error) {
        throw error;
  ...
  Mutation: {
    ...
    confirmEmail: async (args, req, res) => {
    ...
    resturantRequest: async (args, req) => {
      ...
      try {
        ...
        var tokenMail = new Token({ _userId: resturant.id, token: crypto.randomBytes(16).toString('hex') });
        tokenMail.save(async function (err) {
          if (err) {
          ...
          let transporter = nodemailer.createTransport({
            ...
            secure: true,
            auth: {
              user: "bookthechef12@gmail.com", // Your email id
              pass: "asdASD123!@#", // Your password
            },
          });

          // send mail with defined transport object
          let info = await transporter.sendMail({
            from: '"Book the Chef", <mominprofession@gmail.com>',
            to: resturant.email,
            subject: 'Account Verification Link',
            text: 'Hello ' + req.name + ',\n\n' + 'Please verify your account by clicking the link: \nhttp:\/\/' + 'localhost:5000' + '\/confirmation\/' + resturant.email + '\/' + tokenMail.token + '\n\nThank You!\n'
          });

          //  console.log("transporter", transporter, "info", info);
    ...
    setSelectedChef: async (args, req) => {
      try {
        const resturant = await Resturant.findById(req.resturantId);
        if (!resturant) {
          throw new Error('Restaurant not found');
        ...
        // console.log()
        if (resturant.chefs.includes(req.chefId)) {
          await resturant.updateOne({
            selectedChefId: req.chefId
          })

          const updatedResturant = await Resturant.findById(req.resturantId);
    ...
    updateResturantBussinessHourWeek: async (args, req) => {
      try {
        const resturant = await Resturant.findById(req.resturantId);
        if (!resturant) {
          throw new Error('Restaurant not found');
        ...
        // console.log()
        await resturant.updateOne({
          business_hours_week: { ...resturant.business_hours_week, ...req.weekHours }
        })

        const updatedResturant = await Resturant.findById(req.resturantId);
        return {
          ...updatedResturant._doc.business_hours_week
        }
      } catch (error) {
        throw error;
      }
